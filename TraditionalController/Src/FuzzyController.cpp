            ...
            if (this->mapped_er > Domain[0] && this->mapped_er < Domain[domain_size - 1]) {//计算包含在论域内的隶属度
                ...
                for (size_t i = 0; i < e_index_.size(); i++) {
                    ...
                    for (size_t j = 0; j < er_index_.size(); j++) {
                        if (er_index_[j] == -1)
                            continue;
                        this->delat_kp_ += (KpRulesTable[this->e_index_[i]][this->er_index_[j]] *
                                            this->e_memberShip_[i] *
                                            this->er_memberShip_[j]);
                        this->delat_ki_ += (KiRulesTable[this->e_index_[i]][this->er_index_[j]] *
                                            this->e_memberShip_[i] *
                                            this->er_memberShip_[j]);
                        this->delat_kd_ += (KdRulesTable[this->e_index_[i]][this->er_index_[j]] *
                                            this->e_memberShip_[i] *
                                            this->er_memberShip_[j]);
                    }
                }
